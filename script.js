...
// Add to beginning of script.js after DOMContentLoaded
...
function initLoadingScreen() {
    ...
    // Facts rotation
    const facts = [
        "Did you know? Elephants can remember locations for up to 30 years! 🧠",
        "Elephants are the largest land animals on Earth! 🌍",
        "An elephant's trunk has over 40,000 muscles! 💪",
        "Elephants can communicate using infrasound! 🔊",
        "Baby elephants suck their trunks like babies suck thumbs! 👶"
    ];
    
    let factIndex = 0;
    const factsEl = document.getElementById('loadingFacts');
    setInterval(() => {
        factIndex = (factIndex + 1) % facts.length;
        factsEl.innerHTML = `<p>${facts[factIndex]}</p>`;
    }, 3000);
    
    // Skip button
    document.getElementById('skipIntro').addEventListener('click', completeIntro);
...
document.getElementById('continueToSite').addEventListener('click', completeIntro);
...
function initCouponButton() {
    ...
    couponBtn.addEventListener('click', () => {
        couponDropdown.classList.toggle('active');
        couponBtn.textContent = couponDropdown.classList.contains('active') 
            ? '🏷️ Hide Coupons' 
            : '🏷️ Apply Coupon Code';
    });
    
    // Render coupons
    couponDropdown.innerHTML = coupons.map(c => `
        <div class="coupon-item ${c.code === 'NONE' ? 'selected' : ''}" 
             data-code="${c.code}" 
            ...
            ${c.discount > 0 ? `<span class="discount-badge">-${c.discount}%</span>` : '<span>0%</span>'}
        </div>
    `).join('');
    
    // Coupon selection with type checking
    document.querySelectorAll('.coupon-item').forEach(item => {
        item.addEventListener('click', function() {
            ...
            // Check if coupon is valid for cart items
            if (type === 'elephant') {
                const hasElephants = Object.values(cart).some(item => item.type === 'elephant');
                if (!hasElephants && discount > 0) {
                    playSound('error');
                    showNotification('Invalid Coupon ❌', 'This coupon is only valid for elephants!', 'error');
                    document.querySelector('[data-code="NONE"]').classList.add('selected');
            ...
            // Close dropdown
            couponDropdown.classList.remove('active');
            couponBtn.textContent = '🏷️ Apply Coupon Code';
        });
    });
}

// Updated checkout summary calculation with coupon type logic
function updateCheckoutSummaryNew() {
    let subtotal = 0;
    let elephantTotal = 0;
    ...
    let discount = 0;
    if (promoApplied) {
        if (promoType === 'elephant') {
            discount = elephantTotal * (promoDiscount / 100);
        } else {
            discount = subtotal * (promoDiscount / 100);
        }
    }
    ...
    if (document.getElementById('useCoins') && document.getElementById('useCoins').checked && trunkCoins > 0) {
        coinsValue = trunkCoins / trunkCoinsConfig.conversionRate;
...
// Call these on init
initLoadingScreen();
// Call initCouponButton() after proceedToCheckout()
